...
def recruiter_applications(request, job_id):
    ...
    for app in applications:
        ...
        app.interview_score = round(session.final_score, 2) if session and session.final_score else None
...
def candidate_dashboard(request):
    ...
    # Attach interview score to each application
    for app in applications:
        session = InterviewSession.objects.filter(
            candidate=request.user
        ).order_by('-start_time').first()
...
@never_cache
@login_required
def job_rankings(request, job_id):
    if request.user.role not in ['recruiter', 'admin'] and not request.user.is_superuser:
        return redirect('home_redirect')

    job = get_object_or_404(Job, id=job_id)

    # Only the job's recruiter or admin can see rankings
    if request.user.role == 'recruiter' and job.recruiter != request.user:
        return redirect('recruiter_dashboard')

    applications = Application.objects.filter(
        job=job,
        score__isnull=False
    ).select_related('candidate').order_by('-score')

    return render(request, 'accounts/job_rankings.html', {
        'job': job,
        'applications': applications,
    })
